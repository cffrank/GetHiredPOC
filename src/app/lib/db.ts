...
export async function getSavedJobs(env: Env, userId: string): Promise<(Job & { saved_at: number })[]> {
...
): Promise<(Application & { job: Job }) | null> {
  const result = await env.DB.prepare(`
    SELECT
      a.*,
      j.id as job_id,
