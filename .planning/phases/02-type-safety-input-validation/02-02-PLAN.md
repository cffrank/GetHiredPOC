...
8. Check for any OTHER POST/PUT/PATCH routes across all route files that accept JSON bodies (e.g., chat, job-preferences, email-preferences, work-experience, education). If they accept JSON, create schemas for them too. If a route only reads URL params or query strings (no body), no schema needed.
...
CRITICAL: Audit `packages/frontend/src/lib/api-client.ts` to verify the frontend sends fields matching these schemas. The STATE.md blocker says: "Audit actual frontend request shapes before writing Zod schemas." Read the api-client to check what fields each endpoint sends. If the frontend sends extra fields not in the schema, use `.passthrough()` on the Zod schema to avoid rejecting them. If the frontend omits fields the schema requires, make those fields optional.
  ...
  <verify>
- `curl -X POST http://localhost:8787/api/auth/signup -H "Content-Type: application/json" -d '{}'` returns 400 with `{ "error": "Validation failed", "issues": [...] }`
- `curl -X POST http://localhost:8787/api/auth/signup -H "Content-Type: application/json" -d '{"email": "bad", "password": "short"}'` returns 400 with field-level errors for email format and password length
- `cd packages/backend && npx tsc --noEmit` — compiles with no errors
- `npm test --workspace=packages/backend` — existing tests still pass
- `grep -rn "c.req.json()" packages/backend/src/routes/auth.ts packages/backend/src/routes/applications.ts` — zero results on validated routes (replaced with c.req.valid)
  </verify>
  <done>All POST/PUT/PATCH routes with JSON bodies have Zod validation middleware. Malformed requests return 400 with structured field-level error details. Validated routes use c.req.valid('json') for typed body access. Profile route handles dual content-type correctly.</done>
</task>
...
3. `curl` tests against auth/signup and auth/login with empty body → 400 with field-level errors
4. `curl` test against applications POST with missing job_id → 400 with field-level error
5. `cd packages/backend && npx tsc --noEmit` → no errors
6. `npm test --workspace=packages/backend` → tests pass
7. `grep -rn "c.req.json()" packages/backend/src/routes/auth.ts` → zero (using c.req.valid)
</verification>

<success_criteria>
- Zero `: any` in API handler files (route files)
- All 7+ POST/PUT/PATCH JSON body routes have zValidator middleware
